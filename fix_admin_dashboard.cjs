...
// 2. Refresh
content = content.replace(
  /<RefreshCw className="w-4 h-4 mr-2" \/>\s*Refresh/g,
  '<RefreshCw className="w-4 h-4 mr-2" /> {t("admin_refresh")}'
);

// 3. Clear All Data
content = content.replace(
  /<Trash2 className="w-4 h-4 mr-2" \/>\s*Clear All Data/g,
  '<Trash2 className="w-4 h-4 mr-2" /> {t("admin_clear_data")}'
);

// 4. Logout
content = content.replace(
  /<LogOut className="w-4 h-4 mr-2" \/>\s*Logout/g,
...
// We only want to replace it for the worker table and citizen table.
content = content.replace(
  /\{worker\.location\}/g,
