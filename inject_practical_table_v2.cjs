...
const replaced = content.replace(/<\/TabsContent>\s*<\/Tabs>/, "</TabsContent>\n" + tableContent);

if (replaced === content) {
  console.log("Error: Could not inject table content!");
} else {
  fs.writeFileSync('src/frontend/src/pages/AdminDashboardPage.tsx', replaced);
